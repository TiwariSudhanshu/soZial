...
const Rightbar = () => {
  ...
  return (
    <div
      id="rightbar"
      className="h-screen w-1/5 bg-gray-800 text-white flex flex-col fixed right-0"
    >
      {/* Search Bar */}
      ...
      <div className="flex-1 mt-6 px-4 space-y-4">
        <h2 className="text-lg font-semibold border-b border-gray-700 pb-2">
          Suggestions
          <button onClick={fetchUsers} className="absolute right-5">
            <RefreshIcon />
          </button>
        </h2>
        ...
        <button onClick={toggleTheme} className={`px-4 py-2 rounded ml-[40%] `}>
          <DarkModeIcon />
        </button>
      </div>
    </div>
